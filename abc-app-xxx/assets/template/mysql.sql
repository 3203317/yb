...
<#list data_list_pep_db! as doc>  `${(doc.id)!}` <@data_type.m name="${(doc.prop_type)!}" len="${(doc.len_max)?c}"/><#if 1!=doc.is_null> not null</#if> comment '${(doc.prop_desc)!}'<#if doc_has_next>,</#if>
</#list>
);